#'The main function to convert a map from bp to cM
#'
#'The main function to convert a map from bp to cM (and vice versa)
#'
#'@param map a map
#'@param conv a conv object as constructed by make.conv
#'@param cM2bp whether conversion should go from cM to bp
...
cMconvertR<-function(map,conv,named.chr=F, cM2bp = F){
  if (!named.chr){
    nchr<-length(map)
